...
const WORMHOLE_TOKEN_BRIDGE_ADDRESS = "0xDB5492265f6038831E89f495670FF909aDe94bd9" // Official Wormhole Sepolia Token Bridge
...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  console.log(`Using Wormhole Gateway for Sui Testnet (bytes32): ${wormholeGatewayBytes32}`)
  console.log(`Using Wormhole Destination Chain ID for Sui Testnet: ${DESTINATION_CHAIN_ID}`)

  const [btcDepositorWormholeDeployment, proxyDeployment] = await helpers.upgrades.deployProxy(
  ...
  console.log(
    `BTCDepositorWormhole implementation (logic contract for proxy) deployed to: ${await hre.upgrades.erc1967.getImplementationAddress(proxyDeployment.address)}`
  );
  ...
  if (hre.network.name !== "hardhat" && hre.network.name !== "localhost" && proxyDeployment.args) {
    try {
  ...
  if (hre.network.tags.tenderly) {
    ...
    await hre.tenderly.verify({
      ...
      address: await hre.upgrades.erc1967.getImplementationAddress(proxyDeployment.address),
    });
