...
///            is achieved using the LayerZero protocol. First, the `finalizeDeposit`
///            function initiates a LayerZero token transfer that locks the ERC20
///            L1 TBTC within the LayerZero Token Bridge contract and assigns
///            LayerZero-wrapped L2 TBTC to the corresponding `L2WormholeGateway`
///            contract. Then, `finalizeDeposit` notifies the `L2BitcoinDepositor`
///            contract by sending a LayerZero message containing the VAA
///            of the LayerZero token transfer. The `L2BitcoinDepositor` contract
///            receives the LayerZero message, and calls the `L2WormholeGateway`
///            contract that redeems LayerZero-wrapped L2 TBTC from the LayerZero
    ...
    Reimbursable
{
    ...
    ///         the LayerZero Relayer that is responsible to execute the
    ...
    ///
    ///         <depositor-extra-data> L2 deposit owner address in the LayerZero
    ...
    function initializeDeposit(
        ...
        // Convert the L2 deposit owner address into the Bytes32 format and
        // encode it as deposit extra data.
        bytes32 extraData = addressToBytes32(l2DepositOwner);
    ...
    ///      - The call must carry a payment for the LayerZero Relayer that
    ...
    function finalizeDeposit(uint256 depositKey) external payable {
        ...
        emit DepositFinalized(
            depositKey,
            bytes32ToAddress(l2DepositOwner),
            msg.sender,
        ...
        if (address(reimbursementPool) != address(0)) {
            ...
            if (reimbursementAuthorizations[msg.sender]) {
                ...
                // msg.value that covers LayerZero cost, we need to reimburse
    ...
    function quoteFinalizeDeposit(       
        ...
        msgFee = l1OFTAdapter.quoteSend(_sendParam, _payInLzToken);
    }

    /// @notice Transfers ERC20 L1 TBTC to the L2 deposit owner using the LayerZero
    ///         protocol. The function initiates a LayerZero token transfer that
    ///         locks the ERC20 L1 TBTC within the LayerZero Token Bridge contract
    ///         and assigns LayerZero-wrapped L2 TBTC to the corresponding
    ///         `L2WormholeGateway` contract. Then, the function notifies the
    ///         `L2BitcoinDepositor` contract by sending a LayerZero message
    ///         containing the VAA of the LayerZero token transfer. The
    ///         `L2BitcoinDepositor` contract receives the LayerZero message,
    ///         and calls the `L2WormholeGateway` contract that redeems
    ///         LayerZero-wrapped L2 TBTC from the LayerZero Token Bridge and
    ...
    ///      - The appropriate payment for the LayerZero Relayer must be
    ///        attached to the call (as calculated by `quoteFinalizeDeposit`).
    /// @dev Implemented based on examples presented as part of the LayerZero SDK:
    ...
    function _transferTbtc(uint256 amount, bytes32 l2Receiver) internal {
        uint256 minimumAmount = calculateMinimumAmount(amount);

        require(minimumAmount > 0, "minimumAmount too low to bridge");
        require(amount > 0, "Amount too low to bridge");

        MessagingFee msgFee = quoteFinalizeDeposit(wormholeMessageFee);
