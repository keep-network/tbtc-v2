...
contract Bridge is Ownable, EcdsaWalletOwner {
    ...
    /// @dev Requirements:
    ///      - Wallet with `publicKeyX` and `publicKeyY` coordinates  must be in
    ///        `Live` or `MovingFunds` state
    ///      - The challenger must send appropriate amount of ETH used as
    ///        fraud challenge deposit
    ///      - The signature (represented by r, s and v) must be generated by
    ///        the wallet with `publicKeyX` and `publicKeyY` coordinates during
    ///        signing of `sighash`
    ...
    ///         In order to defeat the challenge the same `publicKeyX` and
    ///         `publicKeyY` coordinates and signature (represented by `r`, `s`
    ///         and `v`) must be provided as were used in the fraud challenge.
    ///         Additionally a preimage must be provided which was used to
    ///         calculate the sighash during input signing. The fraud challenge
    ///         defeat attempt will only succeed if the inputs in the preimage
    ///         are considered honestly spent by the wallet. Therefore the
    ///         transaction spending the UTXO must be proven in the Bridge
    ///         before a challenge defeat is called. If successfully defeated,
    ...
    /// @dev Requirements:
    ///      - `publicKeyX`, `publicKeyY` and `sighash` calculated as
    ///        `hash256(preimage)` must identify an open fraud challenge
    ...
    function defeatFraudChallenge(
        ...
        frauds.defeatChallenge(publicKeyX, publicKeyY, preimage, treasury);
    }

    /// @notice Notifies about defeat timeout for the given fraud challenge.
    ///         Can be called only if there was a fraud challenge identified by
    ///         the provided `publicKeyX` and `publicKeyY` coordinates and
    ///         `sighash` and it was not defeated on time. The amount of time
    ///         that needs to pass after a fraud challenge is reported is
    ///         indicated by the `challengeDefeatTimeout`. After a successful
    ///         fraud challenge defeat timeout notification the fraud challenge
    ///         is marked as resolved, the stake of each operator is slashed,
    ///         the ether deposited is returned to the challenger and the
    ...
    ///        for reference
    /// @dev Requirements:
    ///      - `publicKeyX`, `publicKeyY` and `sighash` must identify an open
    ///        fraud challenge
    ...
    function notifyFraudChallengeDefeatTimeout(
        ...
        frauds.notifyChallengeDefeatTimeout(publicKeyX, publicKeyY, sighash);
    }
