...
contract Bridge is Governable, EcdsaWalletOwner, Initializable {
    ...
    /// @param sha256preimage The hash that was generated by applying SHA-256
    ...
    ///        Notice that applying SHA-256 over the `sha256preimage` results
    ///        in `sighash`.  The path from `preimage` to `sighash` looks like
    ///        this:
    ///        preimage -> (SHA-256) -> sha256preimage -> (SHA-256) -> sighash
    ...
    ///        which was calculated from `sha256preimage`
    ///      - Wallet can be challenged for the given signature only once
    function submitFraudChallenge(
        bytes calldata walletPublicKey,
        bytes memory sha256preimage,
        BitcoinTx.RSVSignature calldata signature
    ) external payable {
        self.submitFraudChallenge(walletPublicKey, sha256preimage, signature);
    }
    ...
    ///        `sha256preimage` must identify an open fraud challenge
    ///      - The expression `keccak256(abi.encode(walletMembersIDs))` must
    ///        be exactly the same as the hash stored under `membersIdsHash`
    ///        for the given `walletID`. Those IDs are not directly stored
    ///        in the contract for gas efficiency purposes but they can be
