...
library Frauds {
    ...
    struct Data {
        ...
        /// key built as keccak256(publicKeyX|publicKeyY|sighash).
    ...
    ///        the wallet with public key represented by `publicKeyX` and
    ///        `publicKeyY` during signing of `sighash`
    ...
    function submitChallenge(
        ...
        require(
            CheckBitcoinSigs.checkSig(
                bytes.concat(publicKeyX, publicKeyY),
                sighash,
                signature.v,
                signature.r,
                signature.s
        ...
        uint256 challengeKey = uint256(
            keccak256(abi.encodePacked(publicKeyX, publicKeyY, sighash))
        );
    ...
    ///         In order to finalize the challenge defeat the same `publicKeyX`
    ///         and `publicKeyY` coordinates must be provided as were used in
    ///         the fraud challenge. Additionally a preimage must be provided
    ///         which was used to calculate the sighash during input signing.
    /// @param publicKeyX Wallet's public key's X coordinate
    /// @param publicKeyY Wallet's public key's Y coordinate
    /// @param preimage The preimage which produces sighash used to generate the
    ///        ECDSA signature that is the subject of the fraud claim. It is a
    ...
    ///         that needs to pass after the fraud challenge is reported is
