...
on Bitcoin and 20 confirmations on Ethereum. To avoid problems with a maximum
size of a transaction, the Coordinator should not pick more than 500 deposits.
The Coordinator chooses Bitcoin transaction fee reflecting the current situation
...
plus/minus some margin, and that all deposit UTXOs can be unlocked with the
wallet's public key.
...
to defeat the fraud challenge? Is the maximum 500 transactions in a batch enough
to not risk not being able to submit SPV proof? Check what is the size of a
sweep transaction with 500 inputs and confirm submitting SPV proof and defeating
