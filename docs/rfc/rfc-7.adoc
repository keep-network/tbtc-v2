...
`sweep_max_deposits` wait in the queue for sweeping. Two aspects put a cap on
how many deposits can be swept at once: Ethereum block gas limits and the time
it takes to sign a sweep transaction input. Given that in normal circumstances,
the active wallet is not redeeming or moving funds, the active wallet can focus
on sweeping and optimize its work schedule for this operation, depending on the
current situation in the Bridge. 
