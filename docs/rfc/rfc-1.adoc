...
- `sweeping_refund_safety_time`: The amount of time prior to when a UTXO
  ...
  transaction.
- `sweep_period`: The amount of time we wait between sweep-period-sweeps.
...
aren't enough deposits (maybe because they all got swept in the `sweep_max_btc`
...
setup allows us to have two active wallets that are always staggered from one
