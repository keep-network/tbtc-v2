...
remaining, or exceeds the `wallet_max_age`, the a few different parties can
begin the process of closing it.

If the wallet failed a heartbeat, then the then the <<operator-only,first
operator>> posts a <<operator-only,reimbursable>> transaction to the ethereum
chain declaring an intention to close the wallet. If the wallet exceeds
