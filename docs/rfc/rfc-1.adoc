...
operators going offline. Raising `heartbeat` decreases the
chance that you'll not have enough signers to sign transactions, but decreases
a wallets lifespan and increases overhead. Lowering `dkg` increases
...
first. Any deposit below `dust_threshold` is ignored, for both for triggering a
