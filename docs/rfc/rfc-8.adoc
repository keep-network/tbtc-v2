...
The Wormhole bridging process from L1 to L2 (or sidechain) looks as follows:
...
The Wormhole bridging process from from L2 (or sidechain) to L1 looks as follows:
...
experience seamless, we will implement a minter contract having the authority to
...
The requirement for a seamless user experience is that the Wormhole team has to
integrate the last step into the Wormhole flow, in the same transaction as the
L2 confirmation.
