...
transaction's outputs into `MovingFunds.processMovingFundsTxOutputs`, sums up
the funds and returns a hash of the target wallets to check against
`movingFundsTargetWalletsCommitmentHash` in `notifyWalletFundsMoved`.

If the hashes match, we begin closing the wallet via
`beginWalletClosing`. After `walletClosingPeriod` has elapsed, anyone can call
