...
`BridgeState.activeWalletPubKeyHash`), it is ready for deposits. A user can pay
to a P2(W)SH address with the following Bitcoin script:
...
unique to them, _only they_ know they know that they deposited TBTCv2 until
they reveal that they did so. To the rest of the Bitcoin world, this looks like
a nondescript payment to a meaningless P2SH address. They make this reveal
(which can be done immediately; no need to wait for confirmations) by calling
`Deposit.revealDeposit`.

`Deposit.revealDeposit` takes in the funding transaction, and then the
